import React, { useEffect, useState } from "react";
import { connect } from "react-redux";
import { useNavigate } from "react-router-dom";
import { BASE_URL_1, BASE_URL_2 } from "../constants/urls";
import { get, post } from "../utils/apiHelper";

const Layout = (props) => {
    const [posts, setPosts] = useState([]);
    const [description, setDescription] = useState("");

    const likeHandler = ({ _id }) => {
        console.log("Hello")
        post(`${BASE_URL_1}/post/like/${_id}`, `Bearer ${props.auth.token}`, null, null)
        .then(res => {
            alert('Post liked, refresh page to see changes');
        })
    }

    const commentHandler = ({ _id }) => {
        post(`${BASE_URL_1}/post/comment/${_id}`, `Bearer ${props.auth.token}`, { comment: document.getElementById(`${_id}_comment`).value })
            .then(res => {
                console.log(res);
                document.getElementById(`${_id}_comment`).value = '';
            })
            .catch(err => {
                console.log(err);
            })
    }

    const postHandler = () => {
        let formData = new FormData();
        formData.append('description', description);
        formData.append('image', document.getElementsByName('image')[0].files[0], document.getElementsByName('image')[0].files[0].name);
        post(`${BASE_URL_1}/post/new`, `Bearer ${props.auth.token}`, null, formData);
    }

    useEffect(() => {
        get(`${BASE_URL_1}/post/getall`, `Bearer ${props.auth.token}`)
            .then(res => {
                setPosts(res.data.posts);
            })
            .catch(err => {
                console.log(err)
            })
    }, [])
    const navigate = useNavigate();

    React.useEffect(() => {
        if (!props.auth?.isAuthenticated) {
            navigate("/login");
        }
    }, []);

    return (
        <>
            <main>
                <div className="container">
                    <div className="col-9">
                        {posts.map((post, index) => (
                            <div className="card" key={index}>
                                <div className="top">
                                    <div className="userDetails">
                                        <div className="profilepic">
                                            <div className="profile_img">
                                                <div className="image">
                                                    <img src=
                                                        "https://media.geeksforgeeks.org/wp-content/uploads/20220609093229/g-200x200.png"
                                                        alt="img8" />
                                                </div>
                                            </div>
                                        </div>
                                        <h3>{post.user.name}
                                            <br />
                                            <span>{post.user.address.city}, {post.user.address.state}</span>
                                        </h3>
                                    </div>
                                    <div>
                                        <span className="dot">
                                            <i className="fas fa-ellipsis-h"></i>
                                        </span>
                                    </div>
                                </div>
                                <div className="imgBx">
                                    <img src={post.img}
                                        alt="post-1" className="cover" />
                                </div>
                                <div className="bottom">
                                    <div className="actionBtns">
                                        <div className="left">
                                            <a onClick={() => {
                                                likeHandler(post);
                                            }}>
                                                <span className="heart"
                                                    onclick="addlike()">
                                                    <span>
                                                        <svg aria-label="Like"
                                                            color="#262626"
                                                            fill="#262626"
                                                            height="24"
                                                            role="img"
                                                            viewBox="0 0 48 48"
                                                            width="24">

                                                            <path
                                                                d="M34.6 6.1c5.7 0 10.4 5.2 10.4
												11.5 0 6.8-5.9 11-11.5 16S25 41.3 24
												41.9c-1.1-.7-4.7-4-9.5-8.3-5.7-5-11.5-9.2-11.5-16C3
												11.3 7.7 6.1 13.4 6.1c4.2 0 6.5 2 8.1 4.3
												1.9 2.6 2.2 3.9 2.5 3.9.3 0 .6-1.3 2.5-3.9
												1.6-2.3 3.9-4.3 8.1-4.3m0-3c-4.5 0-7.9
												1.8-10.6 5.6-2.7-3.7-6.1-5.5-10.6-5.5C6 3.1
												0 9.6 0 17.6c0 7.3 5.4 12 10.6 16.5.6.5 1.3
												1.1 1.9 1.7l2.3 2c4.4 3.9 6.6 5.9 7.6 6.5.5.3
												1.1.5 1.6.5.6 0 1.1-.2 1.6-.5 1-.6 2.8-2.2
												7.8-6.8l2-1.8c.7-.6 1.3-1.2 2-1.7C42.7 29.6
												48 25 48 17.6c0-8-6-14.5-13.4-14.5z">
                                                            </path>
                                                        </svg>
                                                    </span>
                                                </span>
                                            </a>
                                            <a href={`/post/comment/${post._id}`}>
                                                <svg aria-label="Comment"
                                                    className="_8-yf5 "
                                                    color="#262626"
                                                    fill="#262626"
                                                    height="24"
                                                    role="img"
                                                    viewBox="0 0 48 48"
                                                    width="24">

                                                    <path clip-rule="evenodd"
                                                        d="M47.5 46.1l-2.8-11c1.8-3.3 2.8-7.1 2.8-11.1C47.5
										11 37 .5 24 .5S.5 11 .5 24 11 47.5 24 47.5c4 0
										7.8-1 11.1-2.8l11 2.8c.8.2 1.6-.6 1.4-1.4zm-3-22.1c0
										4-1 7-2.6 10-.2.4-.3.9-.2 1.4l2.1
										8.4-8.3-2.1c-.5-.1-1-.1-1.4.2-1.8 1-5.2 2.6-10
										2.6-11.4 0-20.6-9.2-20.6-20.5S12.7 3.5 24 3.5
										44.5 12.7 44.5 24z"
                                                        fill-rule="evenodd">
                                                    </path>
                                                </svg>
                                            </a>
                                            <a onClick={() => {
                                                navigator.clipboard.writeText(`${BASE_URL_2}/post/${post._id}`)
                                                alert('Link copied')
                                            }}>
                                                <svg aria-label="Share Post"
                                                    className="_8-yf5 "
                                                    color="#262626"
                                                    fill="#262626"
                                                    height="24"
                                                    role="img"
                                                    viewBox="0 0 48 48"
                                                    width="24">
                                                    <path
                                                        d="M47.8 3.8c-.3-.5-.8-.8-1.3-.8h-45C.9 3.1.3
										3.5.1 4S0 5.2.4 5.7l15.9 15.6 5.5 22.6c.1.6.6
										1 1.2 1.1h.2c.5 0 1-.3
										1.3-.7l23.2-39c.4-.4.4-1 .1-1.5zM5.2
										6.1h35.5L18 18.7 5.2 6.1zm18.7
										33.6l-4.4-18.4L42.4 8.6 23.9 39.7z">
                                                    </path>
                                                </svg>
                                            </a>
                                        </div>
                                        <div className="right">
                                            <svg aria-label="Save"
                                                className="_8-yf5 "
                                                color="#262626"
                                                fill="#262626"
                                                height="24"
                                                role="img"
                                                viewBox="0 0 48 48"
                                                width="24">

                                                <path
                                                    d="M43.5 48c-.4 0-.8-.2-1.1-.4L24 29 5.6
										47.6c-.4.4-1.1.6-1.6.3-.6-.2-1-.8-1-1.4v-45C3 .7
										3.7 0 4.5 0h39c.8 0 1.5.7 1.5 1.5v45c0 .6-.4
										1.2-.9 1.4-.2.1-.4.1-.6.1zM24 26c.8
										0 1.6.3 2.2.9l15.8 16V3H6v39.9l15.8-16c.6-.6
										1.4-.9 2.2-.9z">
                                                </path>
                                            </svg>
                                        </div>
                                    </div>
                                    <a href={`/post/liked/${post._id}`}>
                                        <p className="likes">{post.likes.length} likes</p>
                                    </a>
                                    <a href={`/post/comment/${post._id}`}>
                                        <h4 className="comments">View all {post.comments.length} comments</h4>
                                    </a>
                                    <div className="addComments">
                                        <div className="reaction">
                                            <h3>
                                                <i className="far fa-smile"></i>
                                            </h3>
                                        </div>
                                        <input id={`${post._id}_comment`} type="text"
                                            className="text"
                                            placeholder="Add a comment..." />
                                        <button onClick={() => { commentHandler(post) }}>Post</button>
                                    </div>
                                </div>
                            </div>
                        ))}
                    </div>
                    <div className="col-3">

                        <h3>Add a post</h3>
                        <input type="file" accept=".jpg, jpeg,.png" name="image" />
                        <br />
                        <br />
                        <input name="description" placeholder="Add a description" onChange={(e) => {
                            setDescription(e.target.value)
                        }} />
                        <br />
                        <br />
                        <button type="submit" onClick={postHandler}>Add post</button>

                    </div>
                </div>
            </main>
        </>
    )
}

const mapstatetoprops = (state) => {
    return {
        auth: state.auth
    }
}

export default connect(mapstatetoprops, null)(Layout);